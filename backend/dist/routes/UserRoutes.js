"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UserController = void 0;
var express_1 = require("express");
var UserService_1 = require("../services/UserService");
var AuthenticationService_1 = require("../services/AuthenticationService");
var CommonErrorHandler_1 = require("../services/CommonErrorHandler");
var JwtService_1 = require("../services/JwtService");
var router = express_1.Router();
var path = "/user";
var loginPath = "/login";
var signUpPath = "/signUp";
router.use(path, JwtService_1.checkJwt);
router.route(path + "/account/:accountid")
    .get(CommonErrorHandler_1.routeErrorWrapper(function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var account;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                if (!req.params['accountid']) return [3 /*break*/, 2];
                return [4 /*yield*/, UserService_1.queryUsersWithAccountId(req.params['accountid'])];
            case 1:
                account = _a.sent();
                if (account) {
                    res.send(account);
                }
                else {
                    res.status(404).send();
                }
                return [3 /*break*/, 3];
            case 2:
                res.status(400).send();
                _a.label = 3;
            case 3: return [2 /*return*/];
        }
    });
}); }));
router.route(loginPath)
    .post(CommonErrorHandler_1.routeErrorWrapper(function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var usrLogin, response, user, respObj, e_1;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 3, , 4]);
                usrLogin = req.body;
                return [4 /*yield*/, AuthenticationService_1.login(usrLogin.username, usrLogin.pword)];
            case 1:
                response = _a.sent();
                return [4 /*yield*/, UserService_1.queryUserWithUsername(usrLogin.username)];
            case 2:
                user = _a.sent();
                respObj = { 'token': response, 'accountid': user.accountid };
                res.send(respObj);
                return [3 /*break*/, 4];
            case 3:
                e_1 = _a.sent();
                res.status(400).send();
                return [3 /*break*/, 4];
            case 4: return [2 /*return*/];
        }
    });
}); }));
router.route(signUpPath)
    .post(CommonErrorHandler_1.routeErrorWrapper(function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var signUpDto, result, e_2;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                _a.trys.push([0, 2, , 3]);
                signUpDto = req.body;
                return [4 /*yield*/, UserService_1.signUp(signUpDto)];
            case 1:
                result = _a.sent();
                if (result) {
                    res.status(200).send();
                }
                else {
                    res.status(500).send();
                }
                return [3 /*break*/, 3];
            case 2:
                e_2 = _a.sent();
                res.status(400).send();
                return [3 /*break*/, 3];
            case 3: return [2 /*return*/];
        }
    });
}); }));
router.route(path)
    .post(CommonErrorHandler_1.routeErrorWrapper((function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var user;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                user = req.body;
                return [4 /*yield*/, UserService_1.saveLocalUser(user)];
            case 1:
                _a.sent();
                res.status(201).send();
                return [2 /*return*/];
        }
    });
}); })));
router.route(path + "/changepass")
    .post(CommonErrorHandler_1.routeErrorWrapper((function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var passwordChange;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                passwordChange = req.body;
                return [4 /*yield*/, UserService_1.changePassword(passwordChange)];
            case 1:
                _a.sent();
                res.status(200).send();
                return [2 /*return*/];
        }
    });
}); })));
router.route(path)
    .put(CommonErrorHandler_1.routeErrorWrapper((function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var user;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                user = req.body;
                return [4 /*yield*/, UserService_1.updateUser(user)];
            case 1:
                _a.sent();
                res.status(200).send();
                return [2 /*return*/];
        }
    });
}); })));
router.route(path + "/:id")
    .delete(CommonErrorHandler_1.routeErrorWrapper(function (req, res) { return __awaiter(void 0, void 0, void 0, function () {
    var id;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                id = req.params['id'];
                return [4 /*yield*/, UserService_1.removeUser(id)];
            case 1:
                _a.sent();
                res.status(200).send();
                return [2 /*return*/];
        }
    });
}); }));
exports.UserController = router;
//# sourceMappingURL=UserRoutes.js.map